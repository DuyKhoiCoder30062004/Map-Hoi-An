...
// TODO: Add loading states for map and audio
// TODO: Implement offline support for cached data
...
export default function App() {
  ...
  const handleAuth = async (e, isLogin) => {
    ...
    try {
      const res = await axios.post(`http://localhost:8000${endpoint}`, { username: usernameInput, password: passwordInput });
  ...
  // --- 4. DATA FETCHING ---
  const fetchRestaurants = async () => { const res = await axios.get("http://localhost:8000/api/nearby"); setRestaurants(res.data); };
  const fetchStats = async () => { try { const res = await axios.get("http://localhost:8000/api/stats"); setStats(res.data); } catch(e){} };
  const fetchPackages = async () => { try { const res = await axios.get("http://localhost:8000/api/admin/packages"); setPackages(res.data); } catch(e){} };
  const fetchHistory = async () => { if(user?.id) { try { const res = await axios.get(`http://localhost:8000/api/user/history/${user.id}`); setHistory(res.data); } catch(e){} } };
  ...
  const autoGenerateContent = async () => {
    ...
    if (selectedLangs.length === 0 && !newRest.audio_vi) {
      // Cho phép vượt qua nếu chỉ sinh audio_vi
    } else if (selectedLangs.length === 0) {
      return alert("Vui lòng chọn ít nhất 1 ngôn ngữ!");
    }
    ...
    try {
      // 1. Dịch văn bản với hệ thống tự động Retry 3 lần khi Request quá tải
      const langsToTranslate = selectedLangs.filter(lang => !workingRest[`description_${lang}`]);
      if (langsToTranslate.length > 0) {
        ...
        for (let i = 0; i < 3; i++) {
          try {
            transRes = await axios.post("http://localhost:8000/api/translate", { 
            ...
            lastError = err;
            console.warn(`Lỗi API Dịch thuật lần ${i + 1}/3:`, err.message);
            await new Promise(r => setTimeout(r, 4000)); // Đứng im 4 giây trước khi thử lại
          }
        }
      ...
      const getAudioSafe = async (text, langName) => {
        for (let i = 0; i < 3; i++) {
          try {
            console.log(`Đang gọi ElevenLabs đọc [${langName}]...`);
            const res = await axios.post("http://localhost:8000/api/tts", { text });
            if (res.data.error) throw new Error(res.data.error);
            return res.data.audio_base64; 
          } catch (err) {
            console.error(`❌ Lỗi API ElevenLabs cho [${langName}]:`, err.message);
      ...
      // 2. CHẠY LẦN LƯỢT Audio
      for (const lang of ["vi", ...selectedLangs]) {
        if (!workingRest[`audio_${lang}`]) {
          const textToSpeek = lang === "vi" ? workingRest.description : workingRest[`description_${lang}`];
          if (textToSpeek) {
             const audio_result = await getAudioSafe(textToSpeek, LANGUAGES.find(l => l.code === lang)?.name);
             if (audio_result) {
                 workingRest[`audio_${lang}`] = audio_result;
                 setNewRest({ ...workingRest }); // Cập nhật UI ngay lập tức
             }
          }
        }
      }

      alert("✅ Hoàn tất! Đã tạo thành công Kịch bản và Audio bằng AI.");
    } catch (err) { 
  ...
  const handleSaveRestaurant = async (e) => {
    ...
    try {
      ...
      const res = await method(`http://localhost:8000${endpoint}`, newRest);
  ...
  const handleDeleteRestaurant = async (id, name) => {
    if (window.confirm(`Xóa quán "${name}"?`)) { await axios.delete(`http://localhost:8000/api/restaurants/${id}`); fetchRestaurants(); fetchStats(); }
  };
  // --- HÀM CRUD CHO USER ---
  ...
  const handleSaveUser = async (e) => {
    ...
    try {
      ...
      const res = await method(`http://localhost:8000${endpoint}`, newUserForm);
  ...
  const handleEditUserClick = (userItem) => {
    ...
    // Khi sửa, không hiển thị mật khẩu cũ. Admin có thể nhập mật khẩu mới để đổi.
  ...
  const handlePlayAudioForUser = async (restaurant) => {
    ...
    if (audioData) {
      playAudio(audioData);
      if (user?.id) {
         await axios.post("http://localhost:8000/api/user/history", {
    ...
    return (
      <div style={{fontFamily: 'Arial', background: '#f5f5f5', minHeight: '100vh', padding: '20px'}}>
        <div style={{maxWidth: '1000px', margin: '0 auto'}}>
          ...
          {/* ==========================================
              TAB 1: QUẢN LÝ QUÁN ĂN (Code cũ của bạn chuyển vào đây)
              ========================================== */}
                  ...
                  <div style={{display: 'flex', gap: '15px'}}>
                    ...
                    <div style={{flex: 1}}><label style={styles.label}>Phân quyền</label>
                      <select value={newUserForm.role} onChange={e=>setNewUserForm({...newUserForm, role: e.target.value})} style={styles.input}>
                        <option value="app">Khách Hàng (User)</option>
                  ...
                  <tbody>
                    {usersList.map((usr) => (
                      <tr key={usr.id} style={{borderBottom: '1px solid #eee'}}>
                        <td style={styles.td}>{usr.id}</td>
                        <td style={styles.td}><strong>{usr.username}</strong></td>
                        <td style={styles.td}>
                          <span style={{padding: '3px 8px', borderRadius: '12px', fontSize: '12px', background: usr.role === 'admin' ? '#ffe0b2' : usr.role === 'owner' ? '#c8e6c9' : '#bbdefb'}}>
                            {usr.role === 'admin' ? "👑 Admin" : usr.role === 'owner' ? "🏪 Owner" : "👥 User"}
                          </span>
                        </td>
                        <td style={styles.td}>
          ...
          {adminTab === "owners" && user?.role === 'admin' && (
            <div style={styles.card}>
                ...
                <table style={{width: '100%', borderCollapse: 'collapse', textAlign: 'left'}}>
                  <thead><tr style={{background: '#f5f5f5'}}><th style={styles.th}>ID</th><th style={styles.th}>Tài Khoản</th><th style={styles.th}>Gói Đăng Ký (Active)</th><th style={styles.th}>Quán Đang Sở Hữu</th><th style={styles.th}>Hành Động</th></tr></thead>
                  <tbody>
              ...
              <div style={{display: 'flex', gap: '20px'}}>
                <div style={{flex: 1, borderRight: '1px solid #ddd', paddingRight: '20px'}}>
                   <ul style={{listStyle: 'none', padding: 0, margin: 0}}>
                     {(user?.role === 'owner' ? restaurants.filter(r => r.owner_id === user.id) : restaurants).map(rest => (
                       <li key={rest.id} onClick={() => handleEditClick(rest)} style={{padding: '10px', background: editingId === rest.id ? '#e3f2fd' : '#f9f9f9', border: '1px solid #eee', marginBottom: '5px', cursor: 'pointer', borderRadius: '4px'}}>
                  ...
                  <div key={pkg.id} style={{border: '1px solid #ddd', borderRadius: '8px', padding: '15px', width: '300px', background: '#f9f9f9'}}>
                     ...
                     {user?.role === 'owner' && (
                        <button onClick={async () => {
                          try {
                            await axios.post('http://localhost:8000/api/owner/subscribe', {owner_id: user.id, package_id: pkg.id});
        ...
        <div style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
           <div style={{background: 'white', padding: '20px', borderRadius: '8px', minWidth: '400px', maxHeight: '80vh', overflowY: 'auto'}}>
              ...
              <div style={{display: 'flex', gap: '10px', marginTop: '15px'}}>
                <button onClick={async () => {
                   await axios.put(`http://localhost:8000/api/user/settings/${user.id}`, { settings: user.settings });
