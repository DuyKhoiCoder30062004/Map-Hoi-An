...
def login_user(req: LoginRequest):
    try:
        ...
        # 3. NẾU ĐÚNG MẬT KHẨU -> CẬP NHẬT TRẠNG THÁI ONLINE NGAY
        # Đặt lệnh UPDATE ở đây để ghi nhận "Heartbeat" đầu tiên khi vừa login
        cursor.execute("UPDATE users SET last_active = CURRENT_TIMESTAMP WHERE id = %s", (user['id'],))
        conn.commit() # Quan trọng: Phải có commit thì DB mới lưu thay đổi

        # 4. Đóng kết nối sau khi đã làm xong mọi việc với DB 
        cursor.close() 
        conn.close() 
            
        # 5. Tạo Token và trả về kết quả [cite: 358]
...
def get_online_count():
    try:
        ...
        # Trả về tổng cộng
        return {"online_count": user_count + guest_count}
    except Exception as e:
...
def user_heartbeat(req: dict):
    try:
        user_id = req.get("user_id") # ngoai tru admin
        guest_id = req.get("guest_id")
        ...
        if guest_id:
            # Lệnh "UPSERT": Nếu khách này có rồi thì cập nhật giờ, nếu chưa có thì thêm mới
            cursor.execute("""
                INSERT INTO guest_sessions (guest_id, last_active) 
...
def update_package(package_id: int, req: PackageRequest):
    try:
        from psycopg2.extras import Json
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        
        # Câu lệnh UPDATE dữ liệu dựa trên ID
        query = """
            UPDATE subscription_packages 
            SET name = %s, price = %s, description = %s, features = %s, duration_days = %s 
            WHERE id = %s
        """
        ...
        cursor.close()
        conn.close()
        return {"message": "Cập nhật gói thành công!"}
    except Exception as e:
        return {"error": str(e)}
...
def delete_package(package_id: int):
    try:
        ...
        # Lưu ý: Nếu có Owner đang dùng gói này, Postgres sẽ báo lỗi khóa ngoại (Foreign Key)
        # Bạn có thể xử lý thêm lỗi này nếu muốn
        return {"error": "Không thể xóa gói vì có dữ liệu liên quan hoặc lỗi máy chủ: " + str(e)}
...
def get_system_stats():
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        # Đếm tổng số User (chỉ đếm role = 'user', không tính admin hay owner)
        cursor.execute("SELECT COUNT(*) FROM users WHERE role = 'user'")
        total_users = cursor.fetchone()[0]
        # Đếm tổng số quán ăn
        cursor.execute("SELECT COUNT(*) FROM restaurants")
        total_rests = cursor.fetchone()[0]
        # Đếm tổng số lượt truy cập (dự kiến)
        today_midnight = datetime.combine(datetime.today(), time.min)
        cursor.execute("SELECT COUNT(DISTINCT guest_id) FROM guest_sessions WHERE last_active >= %s", (today_midnight,))
