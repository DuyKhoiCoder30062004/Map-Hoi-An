    ...
    try:
        ...
        cursor.execute("""
            SELECT u.id, u.username, u.password_hash, u.role, u.settings, 
                   COALESCE(p.poi_limit, 1) as poi_limit, COALESCE(p.allowed_langs, '["vi","en","zh","ko","ja"]'::jsonb) as allowed_langs
            FROM users u
            LEFT JOIN LATERAL (
        ...
        return {"message": "Đăng nhập thành công!", "token": token, "role": user['role'], "username": user['username'], "id": user['id'], "settings": user.get('settings', {}), "poi_limit": user.get("poi_limit", 1), "allowed_langs": user.get("allowed_langs", ["vi","en","zh","ko","ja"])}
    except Exception as e:
        return {"error": str(e)}
